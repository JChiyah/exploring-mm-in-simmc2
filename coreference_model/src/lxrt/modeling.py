...
class VisualFeatEncoder(nn.Module):
    ...
    def forward(self, visn_input):
        ...
        output = (x + y) / 2
        # remove this from later layer to check how it works without visual feats
...
class LXRTEncoder(nn.Module):
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        print(f"lang_feats={lang_feats.shape}, visn_feats={visn_feats.shape}")
...
class LXRTFeatureExtraction(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, visual_feats=None,
        ...
        print(f"[modeling.py:1014] mode={self.mode}")
        if 'x' == self.mode:
            return pooled_output
